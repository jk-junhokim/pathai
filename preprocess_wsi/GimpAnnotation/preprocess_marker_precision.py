...
def image_binarization(raw_path, base_path):
    ...
    for file in tqdm(file_list, desc="Annotation penMarker"):
        ...
        # [108, 250, 242] ~ [84, 250, 242]
        
        # tissue_hsv = cv2.inRange(img_hsv, np.array([135, 10, 30]), np.array([170, 255, 255])) # ! need to change tissue color
        # tissue_hsv = cv2.inRange(img_hsv, np.array([84, 250, 242]), np.array([108, 250, 242]))
...
if __name__ == "__main__":
    ...
    raw_path = '../OF_sample_test/OF_annotated_ver' # ! terminal kill
    base_path = '../OF_preprocessed_results/batch_1' # ! update
