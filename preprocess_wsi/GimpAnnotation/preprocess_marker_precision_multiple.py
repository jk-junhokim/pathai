...
def image_binarization(raw_path, base_path):
    ...
    for file in tqdm(file_list, desc="Annotation penMarker"):
        ...
        file_name = file.split('.')[0] # [0]=OF9003_annotated, [1]=tif
        test_num = file_name.split('_')[0] # [0]=OF9003 ,[1]=annotated
        tiff_name =  test_num + ".tif" # OF9003.tif
        
        print('-' * 25 + "Creating Directories..." + '-' * 25)
        print(f"Loading: {file_name}")
        print(f"Creating Sub-Directory: {test_num}")
        ...
        print('-' * 25 + " Saving Version 00...  " + '-' * 25)
        raw_img_path = f"{base_path}/{test_num}/step/00_cropped_original.png"
        ...
        # * set hyperparameter
        dilation_radius = 89
        min_component_size = 60000
        
        # * start image binarization      
        ...
        hsv_black_marker = cv2.inRange(img_hsv, np.array([0, 0, 0]), np.array([166, 175, 80]))
        black_marker_path = f"{base_path}/{test_num}/step/03_hsv_black_marker.png"
        cv2.imwrite(black_marker_path, hsv_black_marker)

        # ! dilation may not be necessary for gimp annotated versions
        dilated_mask = cv2.dilate(hsv_black_marker, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (dilation_radius, dilation_radius)))
        dilated_mask_path = f"{base_path}/{test_num}/step/04_marker_dilated.png"
        ...
        marker_enclosed_mask_path = f"{base_path}/{test_num}/step/05_contour_filled.png"
        ...
        mask_filtered_path = f"{base_path}/{test_num}/step/06_contour_noise_filtered.png"
        ...
        # large_mask_path = f"{base_path}/{test_num}/step/07_mask.png"
        ...
        # inverted_large_mask_path = f"{base_path}/{test_num}/step/08_inverted_mask.png"
        # cv2.imwrite(inverted_large_mask_path, large_mask)
        
        # invert large mask (get non-tumor region)
        large_mask_ = Image.fromarray(mask_filtered)
        large_mask_ = large_mask_.convert("L")
        inverted_large_mask = ImageOps.invert(large_mask_)
        inverted_large_mask_array = np.array(inverted_large_mask)
        inverted_large_mask_path = f"{base_path}/{test_num}/step/07_inverted_mask.png"
        cv2.imwrite(inverted_large_mask_path, inverted_large_mask_array)
        
        # filter noise from black pen marker
        large_marker_filtered = filter_small_components(dilated_mask, min_component_size)
        print('-' * 25 + " Filtered Small Components  " + '-' * 25)
        large_marker_filtered_path = f"{base_path}/{test_num}/step/08_large_marker_filtered.png"
        cv2.imwrite(large_marker_filtered_path, large_marker_filtered)

        # large_marker = get_marker_line(hsv_black_marker)
        # large_marker = get_large_components(dilated_mask)
        # large_marker_path = f"{base_path}/{test_num}/step/09_large_marker.png"
        # cv2.imwrite(large_marker_path, large_marker)
        
        subtract_img = mask_filtered - large_marker_filtered
        subtract_img = np.uint8(subtract_img)
        print('-' * 25 + " Get tumor Region  " + '-' * 25)
        subtract_img_path = f"{base_path}/{test_num}/step/09_subtract_marker_from_mask.png"
        cv2.imwrite(subtract_img_path, subtract_img)

        # filter noise
        # filtered_subtract_img = get_large_components(subtract_img)
        # filtered_sub_path = f"{base_path}/{test_num}/step/10_mask_subtract_marker.png"
        # cv2.imwrite(filtered_sub_path, filtered_subtract_img)
        
        filtered_subtract_img = filter_small_components(subtract_img, min_component_size)
        filtered_sub_path = f"{base_path}/{test_num}/step/10_filter_subtracted_img.png"
        cv2.imwrite(filtered_sub_path, filtered_subtract_img)
        ...
        # print('-' * 25 + " Stop Code Execution  " + '-' * 25)
        # sys.exit()
        
        # original/tumor/non-tumor area
        non_tumor_area = inverted_large_mask
        # tumor_area = filtered_tumor_cell
        tumor_area = filtered_subtract_img
        
        # dcgen_img_path = f"{DCGEN_PATH}/{test_num}/{tiff_name}" # ! change name to orginial cropped image
        dcgen_test_img = Image.open(raw_img_path)
        ...
        non_tumor_path= f"{base_path}/{test_num}/step/11_non_tumor.png"
        ...
        tumor_path= f"{base_path}/{test_num}/step/11_tumor.png"
...
def generate_patches(base_path, patch_size, information_threshold=0.4, label=1):
    ...
    for file in tqdm(file_list, desc="Generating Patches"):
        ...
        if label == 1:
            data_path = f"{base_path}/{file_name}/step/11_tumor.png"
        elif label == 0:
            data_path = f"{base_path}/{file_name}/step/11_non_tumor.png"
        else:
            break
...
if __name__ == "__main__":
    ...
    raw_path = '../OF_sample_test/OF_multiple_annotations' # ! terminal kill
    base_path = '../OF_preprocessed_results/batch_multiple_annotation_1' # ! update
    
    image_binarization(raw_path, base_path)
    
    # generate_patches(base_path, patch_size=256, information_threshold=0.4, label=0)
    # generate_patches(base_path, patch_size=256, information_threshold=0.4, label=1)
