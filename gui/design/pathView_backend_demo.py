...
def pipeline(data):
    ...
    print(f"[{slide_name}] : Preprocess Done | {preprocess_elapsed_time:.2f}초")
    ...
    print(f"[{slide_name}] : Generate Patch Done | {generate_patch_elapsed_time:.2f}초")
    ...
    print(f"[{slide_name}] : Tumor Prediction Done | {prediction_elapsed_time:.2f}초")
    ...
    for step in data_dict.keys():
        ...
        raw_slide = Image.open(slide_path).convert("RGB")
        resized_raw_slide = raw_slide.resize((ACTUAL_WIDTH, ACTUAL_HEIGHT)) # 오리지널 캔버스
        ...
        image_width = X_MAX - X_MIN                 # 유리 슬라이드 내부 검체 이미지의 너비
        left = (ACTUAL_WIDTH - image_width) // 2    # 유리슬라이드 너비 에서 검체이미지를 뺀 값의 절반 (이미지의 시작지점)
        
        origin = np.full_like(resized_raw_slide,(255,255,255), dtype=np.uint8)      # 유리슬라이드 사이즈만큼 흰색 캔버스 생성
        canvas = cv2.cvtColor(np.array(resized_thumbnail_img), cv2.COLOR_BGR2RGB)  

        dh, dw, origin = resize_and_draw(Y_MIN, Y_MAX, left, tmp_path, step, resized_slide_map, canvas, origin)    # 유리슬라이드에 검체 이미지를 복사
    ...
    print(f"[{slide_name}] Save Result Done | {save_elapsed_time:.2f}초")
    
    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"[{slide_name}] All Process Done | {elapsed_time:.2f}초")
...
if __name__ == "__main__":
    ...
    with Pool(processes=workers) as pool:
        for chunk in chunks:
            ...
            end_time = time.time()
            elapsed_time = end_time - start_time
            print(f"Sub Series process time | {elapsed_time:.2f}초")
            
    total_end_time = time.time()
    total_elapsed_time = total_end_time - total_start_time
    print(f"All series process time | {total_elapsed_time:.2f}초")
