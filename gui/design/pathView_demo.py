...
import pathology_viewer_pipeline_demo as pipe
...
class WindowClass(QMainWindow):
    def __init__(self) :
        ...
        self.button_terminate = QPushButton("Stop")
        self.button_terminate.setFont(font_click)
        self.button_terminate.setStyleSheet("border: 3px solid black;")
        self.button_terminate.setMinimumSize(120, 45)
        ...
        left_side.addWidget(self.button_terminate)
        left_side.addSpacing(10)
        ...
        self.button_terminate.clicked.connect(self.buttonTerminateFunction) # stops DL model
    ...
    def buttonTerminateFunction(self):
    ...
    def onTabChanged(self, index):
        if index == 0:  # Viewer tab is selected
            self.scroll_area.setWidget(self.file_displayer)
            # file_names = [file for file in os.listdir(self.folder_path) if file.endswith('.tiff') or file.endswith('.tif')]
            # self.file_displayer.setPlainText('\n'.join(file_names))
        elif index == 1:
            None
        elif index == 2:  # Log tab is selected
            self.scroll_area.setWidget(self.terminal_display)
        else:
            None
